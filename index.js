...
class ForbiddenStringCheckerPlugin {
    constructor(options = {}) {
        this.options = Object.assign({ forbiddenStrings: options.forbiddenStrings || DEFAULT_FORBIDDENSTRINGS, 
            // caseSensitive: options.caseSensitive || false,
            errorMessage: options.errorMessage || '检测到禁止使用的字符规则', include: options.include || /\.(js|jsx|ts|tsx|vue)$/, exclude: options.exclude || /node_modules/ }, options);
        // 存储检查结果
        this.checkResults = {
            warnings: [],
            errors: [],
    ...
    apply(compiler) {
        const { forbiddenStrings, errorMessage, include, exclude } = this.options;
        ...
        compiler.hooks.compilation.tap('ForbiddenStringCheckerPlugin', (compilation) => {
            compilation.hooks.succeedModule.tap('ForbiddenStringCheckerPlugin', (module) => {
                let filename;
                ...
                if (include.test(filename) &&
                    ...
                    patterns.forEach((pattern, index) => {
                        ...
                        if (matches) {
                            // 获取原始字符串用于错误信息
                            const originalString = typeof forbiddenStrings[index] === 'string'
